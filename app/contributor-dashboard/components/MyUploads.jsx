// app/contributor-dashboard/components/MyUploads.jsx
'use client'

import { useState } from 'react'
import { Edit, Trash2, Eye, MoreVertical, X, Save } from 'lucide-react'

export default function MyUploads({ uploads, onUpdate, onDelete }) {
        ...
        <table className="w-full">
          <thead className="bg-[#0d1428] border-b border-[#00d4ff]/10">
            <tr>
              ...
              <th className="px-6 py-3 text-left text-xs font-semibold text-[#00d4ff] uppercase">Downloads</th>
              <th className="px-6 py-3 text-center text-xs font-semibold text-[#00d4ff] uppercase">Actions</th>
            </tr>
          </thead>
          <tbody>
            {uploads.map((upload) => (
              <tr key={upload.id} className="border-b border-[#00d4ff]/5 hover:bg-[#0d1428]/50 transition-colors">
                {editingId === upload.id ? (
                  <>
                    ...
                    <td className="px-6 py-4">
                      <span className="text-gray-400 text-sm">{upload.downloads || 0}</span>
                    </td>
                    <td className="px-6 py-4">
                  ...
                  <>
                    ...
                    <td className="px-6 py-4">
                      <span className={`px-2 py-1 text-xs rounded-full ${
                        upload.status === 'active' ? 'bg-green-500/10 text-green-400' :
                        upload.status === 'pending' ? 'bg-yellow-500/10 text-yellow-400' :
                    ...
                    <td className="px-6 py-4">
                      <span className="text-white text-sm font-semibold">{upload.downloads || 0}</span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-center gap-2">
                        <button
                        ...
                        <button
                          onClick={() => setShowDeleteModal(upload.id)}
                          className="p-2 bg-red-500/10 text-red-400 rounded-lg hover:bg-red-500/20 transition-colors"
                          title="Delete"
                        >
                          <Trash2 size={16} />
